...
import { Eye, EyeOff, Mail, Lock, User, CloudCog, ArrowRight, Loader2, ShieldCheck, RotateCw } from 'lucide-react';
...
export default function Signup() {
  ...
  const { signup, verifySignupOtp, resendOtp, loginWithGoogle } = useAuth();
  ...
  const handleVerifyOtp = async () => {
    ...
    setVerifying(true);
    try {
      await verifySignupOtp(otpEmail, otpValue);
      toast.success('Account created! Welcome to CloudinaryWatch');
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-dark-950 bg-mesh flex items-center justify-center p-4">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 -left-32 w-96 h-96 bg-accent-violet/10 rounded-full blur-[100px]" />
            ...
            {step === 'form' ? (
              <motion.div key="form" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <div className="text-center mb-8">
                  ...
                  <p className="text-dark-400 text-sm">Start managing your Cloudinary assets</p>
                </div>
              ...
              <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                <div className="text-center mb-8">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-accent-cyan/20 border border-emerald-500/30 flex items-center justify-center mx-auto mb-4">
                    <ShieldCheck className="w-8 h-8 text-emerald-400" />
                  </div>
                  <h2 className="text-2xl font-bold text-white mb-2">Verify your email</h2>
                  <p className="text-dark-400 text-sm">
                    We sent a 6-digit code to<br />
                    <span className="text-emerald-400 font-medium">{otpEmail}</span>
                  </p>
                </div>
                ...
                <div className="flex justify-center gap-3 mb-6" onPaste={handleOtpPaste}>
                  {otp.map((digit, i) => (
                    <input
                      ...
                      autoFocus={i === 0}
                      className="w-12 h-14 text-center text-xl font-bold text-white bg-dark-800/80 border border-dark-600 rounded-xl focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50 outline-none transition-all"
                    />
                  ))}
                </div>
